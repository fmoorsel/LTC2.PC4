...
namespace LTC2.Desktopclients.MauiProfileManager.ServiceTasks
{
    public abstract class AbstractInitMonitorTask : IServiceTask
    {
        ...
        protected AbstractInitMonitorTask(
                ...
                if (pipeServer.IsConnected)
                {
                    ...
                    while (proceed)
                    {
                        try
                        {
                            ...
                            if (statusMessage != null) _statusNotifier.Notify(statusMessage);
    ...
    public static class NamedPipeExtensions
    {
        public static bool WaitForConnectionEx(this NamedPipeServerStream stream, int waitTime, CancellationToken cancellationToken)
        {
            ...
            while (retryCount > 0 && !stream.IsConnected)
            {
                ...
                {
                    try { stream.EndWaitForConnection(ar); }
                    catch (Exception er) { e = er; }
                    signal.Set();
                }, null);

                signal.WaitOne(waitTime);
                if (e != null) throw e;
                if (cancellationToken.IsCancellationRequested) retryCount = 0;
                retryCount--;
            }
